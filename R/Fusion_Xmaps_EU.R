...
Gal_2 <- raster("./Maps/Gallaun/1km/bmAg_JR2000_ll_1km_eur.tif")
Gal_2 <- raster("./Maps/Gall_backup/bmAg_JR2000_ll_1km_eur.tif")
...
error <- error[complete.cases(error),]
colnames(error) <- c("Strata", name_layers)
...
colnames(err.var) <- c(names(err.var_c), paste0(name_layers, "_var_w"))
...
biomass.fusion <- function(x) {
  m <- matrix(x, nrow= 1, ncol=n.layers)
  n <- m[,n.layers]
  g <- m[1:(n.layers-1)] + as.matrix(bias[n,])
  g[g < 0] <- 0
  w <- weight[n,1:(n.layers-1)]
  w[is.na(g)]<- NA
